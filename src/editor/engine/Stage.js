...
export default class Stage {
    ...
    copySprite(el, thumb) {
        ...
        Project.recreateObject(
            page,
            name,
            data,
            whenDone,
            page.id == stg.currentPage.id
        );
    }
    ...
    mouseDown(e) {
        ...
        if (ScratchJr.shaking && hitobj && hitobj.id == ScratchJr.shaking.id) {
            ...
            if (
                pt.x - gn(sprname).owner.screenLeft() < 45 &&
    ...
    checkShaking(pt, target) {
        ...
        var dx =
            globalx(gn("deletesprite")) - globalx(ScratchJr.stage.pagesdiv);
        var dy =
            globaly(gn("deletesprite")) - globaly(ScratchJr.stage.pagesdiv);
    ...
    mouseUp(e) {
        ...
        if (!Events.dragged) {
            ...
            if (spr.type == "sprite") {
                ScratchJr.runtime.threadsRunning =
                    ScratchJr.runtime.threadsRunning.concat(spr.threads);
    ...
    removeFromPage(spr) {
        ...
        if (sprite && sprite.id == spr.id) {
            ...
            page.setCurrentSprite(
                sprites.length > 0 ? gn(sprites[0]).owner : undefined
            );
        }
    }
